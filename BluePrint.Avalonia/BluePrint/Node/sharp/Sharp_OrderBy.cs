...
{
    [NodeBaseInfo("正序排序", "列表功能")]
    public class Sharp_OrderBy : NodeBase
    {
        public Sharp_OrderBy(BParent _bParent):base(_bParent) {
            Title = "正序排序";
        ...
        public override string CodeTemplate(List<string> Execute, List<string> PrevNodes, List<ParameterAST> arguments, List<ParameterAST> result)
        {
            ...
            {
                return $"{PrevNodes.join("\r\n")}\r\n    {result[0].ID.GetID()} = {arguments[0].GetUid(false)}.OrderBy(a=>{arguments[1].CodeTemplate}).ToList();{Execute[0]}";
            }
            else {
