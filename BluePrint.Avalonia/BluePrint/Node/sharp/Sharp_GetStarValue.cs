...
{
    [NodeBaseInfo("取第一值", "列表功能")]
    public class Sharp_GetStarValue : NodeBase
    {
        public Sharp_GetStarValue(BParent _bParent):base(_bParent) {
            Title = "取第一值";
        ...
        public override string CodeTemplate(List<string> Execute, List<string> PrevNodes, List<ParameterAST> arguments, List<ParameterAST> result)
        {
            ...
            {
                return $"{PrevNodes.join("\r\n")}\r\n    {result[0].ID.GetID()} = {arguments[0].GetUid(false)}.FirstOrDefault();{Execute[0]}";
            }
            else {
