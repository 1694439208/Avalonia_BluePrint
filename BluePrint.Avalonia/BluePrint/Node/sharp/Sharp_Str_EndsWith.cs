...
{
    [NodeBaseInfo("判断字符串结尾匹配", "表达式(条件)")]
    public class Sharp_Str_EndsWith : NodeBase
    {
        public Sharp_Str_EndsWith(BParent _bParent):base(_bParent) {
            Title = "判断字符串结尾匹配";
        ...
        public override string CodeTemplate(List<string> Execute, List<string> PrevNodes, List<ParameterAST> arguments, List<ParameterAST> result)
        {
            ...
            return $"{a}.EndsWith({LOL_JSON.ToLiteral(b)})";
        }
    }
}
