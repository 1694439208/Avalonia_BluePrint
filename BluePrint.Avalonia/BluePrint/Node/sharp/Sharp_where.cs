...
{
    [NodeBaseInfo("筛选列表", "列表功能")]
    public class Sharp_where : NodeBase
    {
        public Sharp_where(BParent _bParent):base(_bParent) {
            Title = "筛选列表";
...
then
    ...
    {Execute[1]}",
            ...
            {
                return $"{PrevNodes.join("\r\n")}\r\n    {result[0].ID.GetID()} = {arguments[0].GetUid(false)}.Where(a=>{arguments[1].CodeTemplate}).ToList();{Execute[0]}";
            }
            else {
